...
def process_response(claude_messages, system_instruction, max_tokens, user_dir, chat_id, messages, username, model_name, sid=None, stream_enabled=True):
    """
            ...
            with client.messages.stream(
                ...
                for chunk in stream:
                    ...
                    if chunk_text:
                        ...
                        emit("claude_response_chunk", {"chunk": chunk_text, "chat_id": chat_id})
                ...
                elif hasattr(stream, '_final_response'):
                    if hasattr(stream._final_response, 'usage'):
                        usage_metadata = stream._final_response.usage
        else:
            # 非ストリーミングモード
